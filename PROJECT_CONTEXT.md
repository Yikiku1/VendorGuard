...
- `policies/rules/v1.0.0.yaml` 仍保留范围调整前的五条启用规则清单，尚待按新范围改为仅启用 `VEN-001`、`VEN-002`
...
下一小步是收尾 `VEN-001` 并将 YAML 实施范围改为只启用 `VEN-001`、`VEN-002`，随后实现 `VEN-002`；不提前实现前端、RAG 或复杂审批流程。
...
`VEN-001` 至 `VEN-006`、`PR-001` 的领域定义继续保留；`policies/rules/v1.0.0.yaml` 当前仍列出范围调整前的五条启用规则，下一步必须改为仅启用 `VEN-001`、`VEN-002`，其余规则全部延期。Day 1 至 Day 3 已完成并验收。

2026-09-02 已完成创建供应商、创建准入案件、材料元数据登记、案件详情、最小状态迁移 HTTP 接口、版本化 YAML Schema 以及 `VEN-001` 的首批执行行为；合法迁移、非法迁移、采购经理权限隔离、Schema 边界和 `VEN-001` 三条聚焦路径均已验证。下一步先对齐 YAML 的两条启用规则清单，再实现 `VEN-002`。
