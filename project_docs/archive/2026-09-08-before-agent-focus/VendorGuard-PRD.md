...
> 文档状态：一期 MVP 精简基线
>
> 目标：在固定 10 天期限内，以约 65 至 75 小时交付范围明确、可诚实演示的 VendorGuard MVP。十天版本只交付供应商准入主线；采购例外保留为二期领域设计。
>
> 配套文档：[前期调研与需求分析](./VendorGuard-前期调研与需求分析.md)、[10 天计划](./VendorGuard-10天计划.md)

VendorGuard 面向制造企业采购团队。长期产品覆盖供应商准入和采购申请例外；十天 MVP 只审查供应商准入材料，执行确定性风险规则，检索制度与历史案例提供依据，并将风险路由给有权限的人工审批人。

## 产品目标

长期产品包含两个相互关联、可追溯的案件流程：供应商准入决定供应商能否进入 AVL；采购例外决定某一次使用非 AVL 供应商的 PR 是否可以继续。十天 MVP 只交付供应商准入流程；每个准入结论关联结构化模拟事实、规则或检索依据，最终决定由人工完成。

```text
供应商准入：材料 -> 事实 -> 规则命中 -> 引用证据 -> 人工决定 -> AVL/归档
二期采购例外：PR -> AVL 校验 -> 例外规则与证据 -> 人工决定 -> 单次放行/拒绝/到期
```
...
每个供应商申请接受以下演示输入；十天 MVP 使用结构化事实模拟文件解析，并保留来源位置：

1. 供应商基本信息表
2. 营业执照事实（模拟 PDF 页码）
3. 按供应商品类要求提供的质量证书元数据（模拟 PDF 页码）
4. 报价事实（模拟 XLSX 单元格）
5. 可选的历史履约事实（模拟 CSV 行号）

长期设计支持一套固定营业执照 PDF 模板、一套固定报价 XLSX 模板和一套固定履约 CSV 模板。十天 MVP 使用固定结构化事实样例模拟这些来源，保留页码、单元格或行号，不实现真实文件解析。系统不实现 OCR，也不声称支持任意供应商材料。质量证书作为材料完整性输入和附件元数据保存，本期不实现通用证书解析。
...
- OCR、自建或部署重排模型、复杂评测大屏或真实消息队列
- 真实 PDF、XLSX、CSV 解析、采购申请和采购例外执行流程
- 使用真实供应商和业务数据
...
| 角色 | 操作 |
| --- | --- |
| 采购专员 | 创建准入申请、登记材料元数据、补件、查看结果 |
| 采购经理 | 批准或拒绝供应商准入 |
| 质量经理 | 已完成领域定义；证书临期和关键物料审批延期实现 |
| 管理员 | 已完成领域定义；独立管理页面和管理动作延期实现 |

首版保留四个稳定角色代码，但只为采购专员和采购经理提供预置账号并验证访问控制。采购专员不能审批自己提交的准入案件。采购例外、质量经理、管理员和关键物料双人审批属于已定义但未实现能力，不得在演示或简历中写成已交付。
...
策略保存为版本化 YAML。七条规则继续作为领域设计保留，十天 MVP 只实现并验证 `VEN-001`、`VEN-002`；`VEN-003`、`VEN-004`、`VEN-005`、`VEN-006` 和 `PR-001` 标记为延期实现，不进入 10 天完成判定。
...
每个分块至少保存文档版本、生效期、章节或页码和原文。无证据时，系统输出“证据不足”。一期固定使用 10 篇公开或模拟的制度、SOP 和历史案例，实现 PostgreSQL 全文召回、pgvector 向量召回和简单 RRF 融合，并用 15 个固定问题验证引用。公开来源以带出处与抓取日期的文本快照入库，只保存支撑结论的必要片段，不保存整页原站内容，也不解析真实 PDF、XLSX、CSV 文件。

一期检索质量的度量以离线脚本输出 hit@4、MRR、recall@20、无答案精度和引用可定位率，不做指标看板、在线监控或 A/B 平台。精排允许调用外部 rerank API 作为可选步骤并默认关闭，不自建、不部署、不训练重排模型，也不做多厂商切换。权限过滤、增量索引和知识库管理能力不进入一期。
...
| `purchase_requisitions` | 二期保存 PR 的物料、数量、金额、申请人与目标供应商 |
| `procurement_exception_cases` | 二期保存一次 PR 例外的原因、资格快照、批准范围和有效期 |
...
1. **业务流程优先**：准入、审批和审计是一期核心；RAG 只是证据能力，采购例外属于二期。
...
| 文件解析 | 结构化事实 Schema | 使用模拟 PDF 页码、XLSX 单元格和 CSV 行号；真实文件解析与 OCR 延期 |
| Agent 编排 | LangGraph | 只编排准入流程并验证一个恢复场景；大模型不决定路由或最终审批 |
| 模型接入 | OpenAI-compatible 模型适配器 | 延迟初始化、超时和结构化输出；业务模块不直接依赖厂商 SDK |
| RAG | PostgreSQL 全文检索、pgvector、简单 RRF | 基于 10 篇资料的混合召回、引用定位和无证据拒答 |
...
| 交付 | Docker Compose、GitHub Actions | 保留 PostgreSQL Compose；应用按本地命令启动，自动执行检查与测试 |
...
│  ├─ purchase_exceptions/          # 二期 PR 与单次采购例外占位
...
| `purchase_exceptions` | 二期创建 PR、校验 AVL、创建和关闭单次例外案件 | 改变供应商资格；本期不实现 |
...
- 两条启用规则、关键状态迁移和提交人隔离必须有正向、反向与边界测试。
...
- 两个准入简化案例必须使用固定种子数据，可在新环境中重复得到相同结果。
...
| 补件后复核 | 缺少品类必填材料；补齐后重新运行 | 创建补件，补齐后重新运行并保留首次 `VEN-002` 命中 |
...
| --- | --- |
| 两条启用规则固定案例 | 结果与预期一致 |
| 15 个检索问题 | 返回预期引用或“证据不足” |
| 两个 Agent 结构化输出 | 固定测试输入通过 Schema 校验 |
| 指定节点恢复场景 | 已成功节点不重复执行 |
| 提交人与审批人隔离 | 采购专员不能审批自己的案件 |

MVP 完成时，正常准入可从前端走完创建、分析、审批和审计闭环；补件复核至少通过接口流程验证。支持范围内的风险可定位模拟材料来源与制度依据。PostgreSQL Docker Compose 和本地应用命令可以复现核心 Demo。
...
| M2 供应商准入闭环 | Day 3-4 | 最小案件、两条启用规则、采购经理审批和审计 | 使用结构化事实跑通两个准入简化案例 |
| M3 结构化事实、证据与编排 | Day 5-7 | 模拟来源事实、十篇资料混合检索、两个受控 Agent 和一个恢复场景 | 正常准入工作流稳定运行并可从指定失败节点恢复 |
| M4 前端闭环 | Day 8-9 | 三个准入入口、内联证据和一条浏览器冒烟测试 | Playwright 完成正常准入，接口测试覆盖补件和审批 |
| M5 评测与交付 | Day 10 | 小型评测、CI、Docker Compose、README、架构图和演示材料 | 新环境能够按 README 复现核心 Demo，关键缺陷关闭 |

## 后续维护方向

一期交付后，优先实现 `VEN-003`、`VEN-006`、质量经理流程、关键物料双审批和只读系统追踪页，再考虑管理员管理动作、更多文件模板、权限过滤、知识库增量索引、OCR、自建重排模型和评测大屏。真实 ERP、工商或认证机构数据集成应在权限、数据合规与失败恢复机制稳定后再评估。
