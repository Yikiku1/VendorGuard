...
> 说明：本表定义测试题和预期答案，尚未实现规则 YAML、规则引擎或自动化测试。

## 本期实施范围

七条规则及 43 个边界案例继续作为领域设计和后续扩展依据，不删除已有分析。固定 10 天版本只实现并验证 `VEN-001`、`VEN-002`、`VEN-004`、`VEN-005` 和 `PR-001`；`VEN-003` 与 `VEN-006` 属于已定义但延期实现，不计入本期完成判定，也不得在演示或简历中写成已交付。
