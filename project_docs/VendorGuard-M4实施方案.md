...
> 日期：2026-09-15。
> 状态：**已完成（M4-0 至 M4-7，2026-09-16）**。验收清单见第 10 节；完成记录见
> [开发计划](VendorGuard-Agent开发计划.md) 的 M4 段落与 [README](../README.md) 的验收表。
...
状态：已完成（2026-09-16）。三个接口的契约、两处设计取舍（重跑资格与页面按钮同源、
重跑签名没有自选参数）与两条链路实跑见[开发计划](VendorGuard-Agent开发计划.md) 的
M4-4 完成记录。记录层本步未改动。
...
状态：已完成（2026-09-16）。页面资源与路由、样式口径（Linear 风：零渐变零投影、
圆角 4px）、浏览器实测两条限制见[开发计划](VendorGuard-Agent开发计划.md) 的 M4-5 完成记录。
...
状态：已完成（2026-09-16）。四类交互、检索事件新增 `nodes` 保存原文（页面不重新检索）、
四条路径的浏览器实测见[开发计划](VendorGuard-Agent开发计划.md) 的 M4-6 完成记录。
...
状态：已完成（2026-09-16）。六条真实记录（完整 / 补充两轮 / 无依据 / 扫描件 / 真实失败 /
重跑成功）、布局与浏览器检查、README 与五分钟演示脚本见
[开发计划](VendorGuard-Agent开发计划.md) 的 M4-7 完成记录。
