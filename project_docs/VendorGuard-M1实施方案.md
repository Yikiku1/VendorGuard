...
> 进度：步骤 A、B、C 已于 2026-09-08 完成；当前进入步骤 D。
...
| D 有限循环 | 从一次往返扩展到条件循环和追问 | 替身测试覆盖未知工具、坏 JSON、参数错误、无调用假结论、超时和预算上限 | 未开始 |
| E 演示与理解 | 正常、缺项、未知三个真实模型案例 | 记录实际结果，能解释失败并亲手修改一处行为 | 未开始 |
